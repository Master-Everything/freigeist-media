...
import { Scale, Loader2, Copy, Check, ChevronDown, Maximize2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useArticleCheck } from "@/hooks/useArticleCheck";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";
import CheckResultDialog from "./CheckResultDialog";

interface AIJournalismCheckPanelProps {
  content: string;
  postId?: string;
}

const AIJournalismCheckPanel = ({ content, postId }: AIJournalismCheckPanelProps) => {
  const { t } = useTranslation();
  const [copied, setCopied] = useState(false);
  const [historyOpen, setHistoryOpen] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const { result, loading, history, runCheck } = useArticleCheck(postId, "journalism", "check-journalism");
    ...
    <div className="space-y-3">
      ...
      {history.length > 0 && !loading && (
        <p className="text-xs text-muted-foreground">
          {t("ai.journalism.lastCheck")}: {formatTimestamp(history[0].created_at)} {t("ai.checkedBy")} {history[0].display_name}
        </p>
      )}
...
export default AIJournalismCheckPanel;
