...
import { Building2, Loader2, Copy, Check, ChevronDown, Maximize2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useArticleCheck } from "@/hooks/useArticleCheck";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";
import CheckResultDialog from "./CheckResultDialog";

interface AICompanyNewsCheckPanelProps {
  content: string;
  postId?: string;
}

const AICompanyNewsCheckPanel = ({ content, postId }: AICompanyNewsCheckPanelProps) => {
  const { t } = useTranslation();
  const [copied, setCopied] = useState(false);
  const [historyOpen, setHistoryOpen] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const { result, loading, history, runCheck } = useArticleCheck(postId, "company-news", "check-company-news");
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Button size="sm" onClick={handleRun} disabled={loading || !content.trim()} className="gap-1.5">
          ...
          {t("ai.companyNews.run")}
        </Button>
      </div>

      {history.length > 0 && !loading && (
        <p className="text-xs text-muted-foreground">
          {t("ai.companyNews.lastCheck")}: {formatTimestamp(history[0].created_at)} {t("ai.checkedBy")} {history[0].display_name}
        </p>
      )}
...
export default AICompanyNewsCheckPanel;
